...
public class CreditBankAccountRepository(DbContext context) : BaseBankAccountRepository<CreditBankAccount>(context)
{
    ...
    public override async Task<Result<IEnumerable<CreditBankAccount>>> GetAll(Expression<Func<CreditBankAccount, bool>> filter)
    {
        List<CreditBankAccount> res = await _context.Set<CreditBankAccount>()
                .AsNoTracking()
    ...
    public override async Task<Result<CreditBankAccount>> GetOne(Expression<Func<CreditBankAccount, bool>> filter)
    {
        CreditBankAccount? res = await _context.Set<CreditBankAccount>()
                .Include(c => c.Currency)
                .Include(c => c.Term)
                .Where(c => c.DeletedAt == null)
